...
theoparse inputstr =
  let tokens = alexScanTokens inputstr
   in theoparsefile (theoparser tokens)
...
theoparseSubtheo (((subtheotopdec, subtheo), (decanatheo, cancel)), tab) (signature, canceqs, [], decanas) =
  let ops = parseSig signature
   in ( ( (subtheotopdec, \x -> if x `elem` ops then [] else subtheo x),
          (decanatheo ++ (parseDecana decanas), parseCancel canceqs cancel)
        ),
        tab
      )
theoparseSubtheo
  (((subtheotopdec, subtheo), (decanatheo, cancel)), tab)
  (signature, canceqs, topdecs, decanas) =
    let ops = parseSig signature
        (newtheo, ops') = theoparseTopdec topdecs ops
        table = parseSigPass2 signature tab ops'
     in ( ( ( \x ->
                if x `elem` ops
                  then newtheo
...
parseCancel pequations (eqs, theoryOf) =
  let allterms = concatMap (\(x, y) -> x : y : []) pequations
      ...
      neweqs =
        map
          (\(x, y) -> (ppterm register x, ppterm register y))
          pequations
...
parseSig ((op0, arity) : signature) =
  let op = stringToOp op0
      ops = parseSig signature
   in if op0 == "napair"
        then
          errorth "Overloading the builtin symbol 'napair' is not permitted."
        else
          if (arity < 0)
            then errorth ("Operator " ++ op0 ++ " has arity<0.")
            else
              if (arity > 2)
                then errorth ("Operator " ++ op0 ++ " has arity>2.")
                else
                  op : ops
...
parseSigPass2 ((op0, arity) : signature) tab0 ops =
  let table = parseSigPass2 signature tab0 ops
   in makeEntry
        table
        op0
        arity
        ...
        map
          ( \topdecs ->
              let (ident, _, _) = head topdecs
                  op = stringToOp ident
               in if (not (op `elem` ops))
                    then errorth ("Operator " ++ ident ++ " undeclared in subtheory.")
                    else
...
parseTopdecPattern (PCompT operator [PVar vident1, PVar vident2], cases) ops =
  let register = foldl registerVar initialRegister [vident1, vident2]
      ...
      op = stringToOp operator
   in if (not (op `elem` ops))
        then errorth ("Operator " ++ operator ++ " undeclared in subtheory.")
        else
  ...
  case term of
    PCompT operator [PVar vident1, PVar vident2] ->
      let i = lookupVar register ident
          ...
          [v1, v2] = map (lookupVar register') [vident1, vident2]
          op = stringToOp operator
       in (Conditional (i, (op, [v1, v2])) (parseTopdecCases ccases register'))
            : (parseTopdecCases cases register)
    PCompT operator [PVar vident] ->
      let i = lookupVar register ident
          ...
          op = stringToOp operator
       in (Conditional (i, (op, [v, vclone])) (parseTopdecCases ccases register''))
            : (parseTopdecCases cases register)
