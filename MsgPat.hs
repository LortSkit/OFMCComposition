...
initialState ik0 =
  if lversion
    then ((\(x, y) -> x ++ y) . lanalysis . (map mklabel)) ik0
    else
      let ik = analysis ik0
       in map (\x -> (x, reshape ik x)) ik
  ...
  if lversion
    then
      ...
      let ik0 = map fst state
          ik = analysis (msg : ik0)
          nik = ik \\ (msg : ik0)
       in (map (\(m, p) -> (m, reshapePat ("In receiveMsg\n" ++ (show msg) ++ "\n" ++ (show state)) ik (m, p))) state)
...
reshape ik (msg@(Comp f xs)) =
  if f == Exp || f == Xor
    then case [ (f', xs')
                | (Comp f' xs') <- eqMod eqModBound stdTheo [msg],
                  f == f',
                  all (indy ik) xs'
              ] of
      [] -> mkname msg
      ((f', xs') : _) -> Comp f' (map (reshape ik) xs')
    else case (opening msg) of
...
reshapePat0 ik (msg@(Comp f xs), pat@(Atom a)) =
  if f == Xor
    then if all (indy ik) xs then Just (Comp Xor (map (reshape ik) xs)) else Just (Atom a)
    else case (opening msg) of
            ...
            then
              ...
              if f == g
                then
                  ...
                  if f == Apply && g == Cat
                    then error "It is apply and Cat"
...
opening msg =
  case msg of
    ...
    Comp Pseudonym ms -> Just [Atom "no you don't"]
    Comp AuthChan ms -> Just ms -- [Atom "no you don't"]
    Comp ConfChan ms -> Just ms -- [Atom "no you don't"]
...
getProblem ik (Comp f xs) ind =
  concat
    [ ind
        ...
        ++ (getProblem ik x (ind ++ "|"))
      | x <- xs,
        isNothing (synthesisPattern ik x)
    ]
  ...
  case getElem m ik of
    ...
    Nothing ->
      case m of
        ...
        Comp Xor list ->
          let ik0 = map fst ik
           in if synthesizable ik0 m
                then
                  if (all (synthesizable ik0) list)
                    then
...
enlabel ((Comp f xs), (Comp g ys)) =
  if f /= g || (length xs) /= (length ys)
    then error "error in function enlabel"
    else
  ...
  if lversion
    then
      ...
      let ik0 = map fst state
          ik = analysis (msg : ik0)
          nik = ik \\ ik0
       in (map (\(m, p) -> (m, reshapePat "" ik (m, p))) state)
...
reshapePatAna (msg, msgp) ik =
  let mmap = allposL (msg, msgp)
      lookup m =
        let mp = [p | (m', p) <- mmap, m == m']
         in if null mp then error ("did not find " ++ (ppMsg Pretty m)) else head mp
   in map (\m -> (m, lookup m)) ik

allposL :: LMsg -> [LMsg]
allposL (mp@(Comp f xs, Comp g ys)) =
  if f == g
    then mp : (concatMap allposL (zipl xs ys))
