...
fAct formats ((sp@(_, _, _), ChannelProtocol id, _), _, _, _) = error ("-" ++ id ++ "-> symbol only allowed when using --vert flag!")
          ...
          let (_, _, _, _, _, goals) = protocol pts
              folder (ChGoal ((speer@(sender, _, _)), channeltype, (rpeer@(receiver, _, _))) msg) rs =
                let (i, pattern) = findFirstKnow rs sender msg 1
                    ...
                    (l, [], f, r) = head post
                    newfacts =
                      ( if channeltype == Secure
                          || channeltype == Confidential
                        ...
                        ++ ( if channeltype == Secure
                               || channeltype == Authentic
                               || channeltype == FreshAuthentic
                               || channeltype == FreshSecure
                               then
                                 [ Fact
                                     "witness"
           ...
           in {-
              	...
              	in rs'
              -}
              pts {rules = foldr folder rs goals}
...
findLastKnow ((l, [], f, r) : rules) ident msg i maybe_n_pat =
  case getKnow r ident of
    ...
    Just know ->
      maybe
        (findLastKnow rules ident msg (i + 1) maybe_n_pat)
...
ppFact outf (State "dummy" msgs) = "state_dummy" ++ "(" ++ (ppMsgList outf (reorder (map snd msgs))) ++ ")" -- only used with --vert flag
