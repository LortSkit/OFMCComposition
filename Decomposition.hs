...
metaTopdec theo op (BinOp op' t1 t2) =
  let interpret [] termlists sub = []
      ...
      interpret ((Conditional (v, (op, vars)) uncases) : cases) termlist sub =
        let k = length termlist
         in if v >= k
              then error "decomposing nonexisting var"
              else
                if vars /= [k .. (k + ((length vars) - 1))]
                  then error ("decomposition unexpected vars " ++ (show vars) ++ " " ++ (show [k .. (k + (length vars))]))
                  else
                    let tds = metaTopdec theo op (termlist !! v)
                     in (concat [interpret uncases ((map (substitute sub') termlist) ++ ts) (mergesub sub sub') | (ts, sub') <- tds])
                          ++ (interpret cases termlist sub)
   in concat [interpret cases [t1, t2] Map.empty | ((op'', [v1, v2]), cases) <- theo op, op' == op'']
metaTopdec _ _ _ = []
...
unifyF fecTheo sub ((BinOp op m1 m2, BinOp op' n1 n2) : eqs) =
  let subtheo = (snd fecTheo) op
   in if (subtheo == []) || (not (op' `elem` subtheo))
        then
...
decana2 decanaTheo fecTheo m =
  let ruleloop
        (term, ineq, key, result) =
          map
            (\(sub, _) -> (substitute sub key, substitute sub result, dropRuleVars sub))
            ( checksubstineqs
                fecTheo
                ineq
...
checkineq fecTheo ineq@(Negsub vars disjunction) =
  case ( unifyF
           ...
           Map.empty
           ( map
               (\(x, y) -> (freshfree vars x, freshfree vars y))
               disjunction
           )
       ) of
    [] -> ineq
...
checksubstineqs fecTheo ineqs =
  (filter (isSatisfiable . snd))
    . ( map
          ( \sub ->
              if checksub2 (Map.toList sub)
                then error ("Now what is this: " ++ (show_sub sub) ++ " inequality: " ++ (show ineqs))
...
bunormCF theo@(fecTheo, _) j w@(t, sub, ineq) =
  if j > stop
    then error ("nontermination? " ++ (shownormaliz w))
    else case superfstRuleApp theo j w of
      Just (t', s, i, v) ->
        (bunormCF theo (j + 1) (t', s, i))
          ++ ( let nineq = checkineqs fecTheo ((negsub v s) : ineq)
                in if isSatisfiable nineq then (bunormCF theo (j + 1) (t, sub, nineq)) else []
             )
      Nothing -> [w]
...
fstRuleApp fecTheo j ((lhs, rhs) : eqs) (t, sub, ineq) =
  if (j > stop)
    then error "fstRuleApp: nontermination?"
    else case checksubstineqs fecTheo ineq (unifyF fecTheo sub [(t, lhs)]) of
...
removeRuleVars fecTheo (t, sub, ineq) =
  ...
  let list = filter (not . isRuleVar . fst) (Map.toList sub)
   in let varslist = map (\(x, t) -> (x, msg_vars t)) list
          vars = concatMap (\(x, vs) -> x : vs) varslist
          varu =
            filter
              (((/=) []) . snd)
              (map (\(x, vs) -> (x, filter isRuleVar vs)) varslist)
       in if checksub2 list
            then error ("normalize this: " ++ (shownormaliz (t, sub, ineq)))
            else
              if varu == []
                then Just (t, (makesub list), ineq, [])
                else
                  --- check that all variables in the domain of the substitution
                  --- are below expansion limit
                  if (filter (isFresh . fst) varu) /= []
                    then Nothing
                    else
                      let --- the following cannot return @Nothing@ since we have checked that
                       ...
                       in if (length olvars) > (length nuvars)
                            then error "Improper renaming"
                            else
                              if ((nuvar + 2) `elem` vars)
                                then Nothing
                                else
                                  if ((nuvar + 3) `elem` vars)
                                    then Nothing
                                    else
                                      ...
                                      if checksub nusub
                                        then
                                          error
                                            ( "normalize this: "
                                                ++ (shownormaliz (t, sub, ineq))
                                                ++ "--- I would do "
                                                ++ (show nusub)
                                            )
                                        else
                                          let mnusub = makesub nusub
                                              nineqs = substineqs fecTheo mnusub ineq
                                           in if isSatisfiable nineqs
                                                then
...
checksub sub =
  let varsleft = map fst sub
      ...
      allvars = varsleft ++ varsright
   in ((filter (\x -> x `elem` varsright) varsleft) /= [])
...
groundnormalizationCF algTheo@(fecTheo, (_, (_, theoryOf))) t@(BinOp op m1 m2) =
  case theoryOf op of
    Nothing ->
      BinOp
        op
        (groundnormalizationCF algTheo m1)
...
reduceRedices thO m@(BinOp op t1 t2) lhsSubterms =
  (if m `elem` lhsSubterms then (myirreduc thO op m) else [])
    ++ (reduceRedices thO t1 lhsSubterms)
    ++ (reduceRedices thO t2 lhsSubterms)
