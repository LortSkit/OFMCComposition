...
extractRules [] = []
extractRules (x : xs) = case x of
  PRuleSec ys -> map snd ys
  _ -> extractRules xs
...
getfirstrules list =
  let list' =
   ...
   in if (length list') /= 1
        then error ("There are " ++ (show (length list')) ++ " rule sections")
  ...
  let l =
   ...
   in if (length l) /= 1
        then
          ...
          let goals = head l
           in if goals == []
                then error "No goals declared!"
                else ---(error . show)
                  (map goal2rule goals)

dummy_state :: Maybe PIdent -> PFact
dummy_state (Just name) = ("state_dummy", [PConst "6*i", PConst name, PConst "6*i", PConst "0*0", PConst "18*18"])
...
pprintProt algtheo cd normalize (renaming, sections) =
  let (ruleSec) = getfirstrules sections
   ...
   in ( pprintInitialState algtheo initSec,
...
pprintInitialState (_, tab) cnstate =
  if (forgetPos (fst cnstate)) /= []
    then error "Initial state has negative predicates!"
    else
...
preproConds (((PEq (PVar v) t) : conds), sub) =
  let sub' = Map.insert v t sub
   in preproConds (map (ifsubcond sub') conds, sub')
            ...
            let (id, ((ls0, aem0), rs0, fresh_stuff)) = (goal2rule ("execCheck", ([Plain ("iknows", [PCompT "reach_" [PVar "1172", PConst $ "rule" ++ (show number)]])], [])))
             ...
             in ( \(lhs', neg', ineq', rhs', rtype) ->
                    let vars = (var_rule (lhs', neg', ineq', rhs', rtype)) \\ (map (rename . fst) fresh_stuff)
                        ...
                        f2 = substineqs freeAlg sub
                     in ( f1 lhs',
                          f1 neg',
                          f2 ineq,
                          replace_fresh_stuffi rename fresh_stuff (f1 rhs') number,
                          rtype
                        )
                )
                  rhss
...
pprintRules (algtheo, table) cd normalize abstract rename rulez@(((id, ((ls0, aem0), rs0, fresh_stuff)), number) : rules) =
  let (aem, ifsub) = preproConds (aem0, Map.empty)
   ...
   in ( map
              ...
              let vars = (var_rule (lhs', neg', ineq', rhs', rtype)) \\ (map (rename . fst) fresh_stuff)
               ...
               in ( \i ->
                      ( f1 lhs',
                        f1 neg',
                        f2 ineq,
                        replace_fresh_stuffi rename fresh_stuff (f1 rhs') i,
                        rtype
                      )
                  )
          )
        ...
        ++ (pprintRules (algtheo, table) cd normalize abstract rename rules)

normalizeRHS :: AlgTheo -> [Int] -> Rule -> [Rule]
normalizeRHS algtheo@(_, (_, (canceqs, thO))) fresh_stuff (lhs, neg, ineq, rhs, rtype) =
  let subtermslhs = concatMap subtermsF lhs
   in case (partition isMTerm rhs) of
          ...
          map
            ( \(msg, sub, ineq') ->
                ( map (substitute_fact sub) lhs,
                  ...
                  ineq',
                  ( map
                      ((groundnormalize_fact algtheo) . (substitute_fact sub))
                      ((M i real off recv msg ses) : facts')
                  ),
                  rtype
                )
            )
            ( filter
                ...
                ( normalizationCF
                    ...
                    ( mterm,
                      Map.empty,
                      ineq
                        ++ (concatMap ((noredexAny canceqs) . Var) (msg_vars mterm))
                        ++ ( let lhsirredices = (reduceRedices thO mterm (subtermslhs ++ (map Var (msg_vars mterm))))
                              in lhsirredices
                           )
                    )
                )
...
ischNumber (PConst (c : cs)) =
  if (isDigit c)
    then
      if elem '*' cs
        then
          let sndpart = tail (dropWhile ((/=) '*') cs)
           in ischNumber (PConst sndpart)
        ...
        then list
          ...
          if (length ws) /= 1
            then
              ...
              if (length ms) /= 1
                then error ("Wrong number of M-facts " ++ (show ms))
                else
                  let [(W step init resp m1 m2 ses)] = ws
                      [(M _ _ _ _ m _)] = ms
                   in ( (W step init resp m1 m2 ses)
                          : ( if isLhs
                                then
                                  ...
                                  M 0 init init (UnOp opAgent (atom_i)) m ses
                            )
                          : list2
                      )

pprintFact tab (op, ts) =
  if isPrefixOf "state" op
    then
      let role = drop 6 op
          nums = (catMaybes (map ischNumber ts)) ++ [99]
       in if (length nums) < 1
            then error ("There are not enough numbers in the state fact " ++ (show ts))
            else
              if (length ts < 3)
                then error ("Too little information in state " ++ (show ts))
                else
                  ( W
                      (head nums)
      ...
      if op == "error"
        then
          let msg = case ts of
                [PConst m] -> m
                _ -> "error"
           in (BinOpF atom_error (Atom 0 msg) atom_error)
        else
          if op == "iknows"
            then
              let [m1] = ts
                ...
                then
                  ...
                  if op == "contains"
                    then case ts of
                      ...
                      _ ->
                        let ts' = map (pprintTerm tab) ts
                         in ( BinOpF
                                (atom_contains)
                                (head ts')
                                ( foldl
                                    (\x y -> BinOp opPair x y)
                                    (head (tail ts'))
                                    (tail (tail ts'))
                                )
                            )
                    ...
                    -}
                      ...
                      if (op == "witness") || (op == "switness") || (op == "wwitness")
                        then case ts of
                          (m1 : m2 : m3 : m4 : []) ->
                            ( BinOpF
                                atom_witness
            ...
            let role = drop 6 op
             ...
             in if (length nums) < 1
                  then error ("There are not enough numbers in the state fact " ++ (show ts))
                    ...
                    if (length ts < 3)
                      then error ("To few information in state " ++ (show ts))
                      else
                        ( W
                            (head nums)
                            (pprintTerm tab (head ts))
...
pprintTerm tab (PConst a) =
  let (i, s) = span isDigit a
   in if i == []
        then Atom atom_incomparable (if (head s) == '*' then error "!!!" else s)
        else
          ( Atom
              (read i)
              ( if s == []
...
prune :: [PTerm] -> [PTerm]
prune [x] = [PConst "Etc"]
prune (a@(PCompT x args) : xs)
  | x == "public_key" = prune xs
...
modProt prot =
  let f = basenamecheck (getnaming basename (nub (idProt prot)))
      ...
      getnaming i [] var = "0"
      getnaming i (x : xs) var =
        if (var == x)
          then show i
          else getnaming (i + 1) xs var
      getnaming2 i [] var = 0
      getnaming2 i (x : xs) var =
        if (var == x)
          then i
          else getnaming2 (i + 1) xs var
      stepping =
        foldr
          (\(term, step) f -> \x -> if x == term then step else f x)
...
typeTerm ls (PCompT name args) =
  let iter [] x = PCompT x (map (typeTerm ls) args) --- iter::[(Pid,Pid)]->Pid->PTerm
   ...
   in iter ls name
typeTerm ls (PConst name) =
  let iter [] x = PConst x --- iter::[(Pid,Pid)]->Pid->PTerm
      iter ((n, t) : nts) x = if n == x then PCompT t [PConst x] else iter nts x
   in iter ls name
typeTerm ls (PVar name) =
  let iter [] x = PVar x --- iter::[(Pid,Pid)]->Pid->PTerm
...
substVar [] y = PVar y
substVar ((x, Just t) : ss) y
  | x == y = t
  | otherwise = substVar ss y
substVar ((x, Nothing) : ss) y
  | x == y = PVar y
  | otherwise = substVar ss y
...
ifparser algtheo typing cd normalize inputstr =
  let tokens1 = alexScanTokens inputstr
   ...
   in ((pprintProt algtheo cd normalize) . modProt . typeTrafo . parser) tokens1
