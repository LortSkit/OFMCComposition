      ...
      if isIntruder real_sender
        then
          ( if isIntruder official_sender
            ...
            ++ ( if session == Number 17
                   then " can produce secret "
                   else " -> (" ++ (show receiver) ++ "," ++ (show session) ++ "): "
               )
        else
          "("
            ++ (show real_sender)
  ...
  showsPrec p e =
    case e of
      ...
      (Request a b c d bool) ->
        showString
          ( (if bool then "request" else "wrequest")
              ++ "("
              ++ (show a)
              ++ ","
              ++ (show b)
...
substitute_state fecTheo sub s =
  s
    { facts = map (substitute_fact sub) (facts s),
                ...
                case catMaybes [(unify_facts freeAlg wfact lhswterm Map.empty)] of
                  ...
                  sub0 ->
                    let otherterms' = concatMap (\s0 -> map (substitute_fact s0) otherterms) sub0
                        subfacts =
                          concatMap
                            ( \s0 ->
                                unify_factset
                                  fecTheo
                                  (map (substitute_fact s0) otherterms)
                                  rest
                                  s0
                            )
                            sub0
                     in if por && rtype
                          then
                            let (afa, bfa) = partition ((==) wfact) (popo state)
                             in if (afa == [])
                                  then
                                    ( map
                                        ( \(remainingfacts, sub1) ->
                                    ...
                                    substitute_interstate
                                      ...
                                      ( state
                                          { porflag = False,
                                            ...
                                            popo =
                                              ( if rtype
                                                  then overridden ++ (popo state) ++ rest
                                                  else popo state
                                              )
                                          },
                                        (lhs, neg, cond, rhs, rtype)
          ...
          ( \f1 f2 ->
              case (f1, f2) of
                ((W step1 sender1 receiver1 stk1 ltk1 ses1), (W step2 sender2 receiver2 stk2 ltk2 ses2)) ->
                  let cmp = compses ses2 ses1
                   in case cmp of
                        EQ ->
                          if (step1 == step2)
                            then case (stk1, stk2) of
...
unify_facts0 fecTheo (BinOpF m1 m2 m3) (BinOpF m1' m2' m3') sub1 =
  ( unify_facts0
      fecTheo
      (W 0 m1 m2 m3 (atom_dummy) (atom_dummy))
      (W 0 m1' m2' m3' (atom_dummy) (atom_dummy))
  )
    sub1
...
unify_factset0 fecTheo f (s : set) rest sigma =
  ( map
      ( \sigma ->
          let siggi = substitute_fact sigma
           in ((map siggi set) ++ (map siggi rest), sigma)
      )
...
generate_check_state very_lazy fecTheo (state, rule@(lhs, neg, cond, rhs, rtype)) =
  let list = gencheck fecTheo (cs state) Map.empty
   in if null list
        then []
        else
          if very_lazy
...
match_msg_fact very_lazy fecTheo currstate@(state, (lhs, neg, cond, rhs, rtype)) =
  let (constraints, ineq) = (cs state)
   ...
   in case lhsmterms of
        [] ->
          let notmterms = filter isMTerm neg
           in case notmterms of
                [] ->
                  if (checkNIKempty (ik state))
                    then
        ...
        [M i real off recv mterm ses] ->
          let notmterms = filter isMTerm neg
           in case notmterms of
                [] ->
                  generate_check_state
                    very_lazy
                    fecTheo
                    ( state
                        { cs =
                            ( constraints ++ [mkDFromState [mterm] (ik state)],
...
match_rest fecTheo (state, (lhs, neg, cond, rhs, rtype)) =
  let domain =
      ...
      subs =
        concatMap (\neg_fact -> catMaybes ((\x -> map (\y -> unify_facts fecTheo x y Map.empty) (facts state)) (neg_fact))) neg
   in --- concatMap (\x -> catMaybes $ map (\y -> unify_facts fecTheo x y Map.empty) (facts state)) neg
      if (containsnonIDsub domain (map Map.toList subs))
        then []
...
perform_step sfb (state, (lhs, neg, cond, rhs, rtype)) =
  ( state
      { time = (time state) + 1,
        facts =
          union
            (filter (\x -> (not (isMTerm x)) && (sff sfb x)) rhs)
            (facts state)
      },
    (lhs, neg, cond, rhs, rtype)
  )
  ...
  let mterms = filter isMTerm rhs
   ...
   in if mterms == []
        then
          ...
          let ana =
                if veryLazy
                  then [(cs state, Map.empty, ik state)]
...
firstNonSimpleConstraint ((a, b, c) : xs) =
  if (filter (not . isVar) a) /= []
    then (Just (a, b, c), xs)
    else
      let (sc, cs) = firstNonSimpleConstraint xs
       in (sc, (a, b, c) : xs)
...
successors very_lazy theo@(fecTheo, _) por sfb rule s =
  --- was: state
  let r = new_var_rename s (rule (getTime s))
   in ( if very_lazy
          then
            concatMap
              ( \(state, _) ->
        ...
        ( compList
            [ (analz_knowl very_lazy theo por) . (perform_step sfb),
              match_rest fecTheo,
              match_msg_fact very_lazy fecTheo,
              normalize_step,
              match_lhs_waiting_term fecTheo por
            ]
            (s, r)
        )
...
perform_constraint_reduction (algtheo@(fecTheo, _)) s =
  map
    (\(cs', sub) -> substitute_state fecTheo sub s {cs = cs'})
    (reduce algtheo (cs s))
            ...
            concatMap
              ( \((constraints, sub), currentIK) ->
                  let (t, ik, nik) =
                      ...
                      nik' = diffMT nik (fst (getallIK currentIK))
                   in concatMap
                        ( \(constraints', sub', currentIK') ->
                            let ikstate =
                                  if isEmptyMT ik
                                ...
                                constraint' = mkDFromState t ikstate
                             in zip
                                  ( gencheck0
                                      fecTheo
                                      []
...
successors_must theo@(fecTheo, _) por sfb rule s =
  let r = new_var_rename s (rule (getTime s))
   in ( concatMap
          ((map fst) . (analz_knowl very_lazy theo por))
          ( concatMap
              (perform_step_must sfb)
              ( concatMap
                  (match_rest_must)
                  ( concatMap
                      (match_msg_fact_must fecTheo)
                      ( concatMap
                          normalize_step
...
normalizeHC theo sfb (clause : clauses) (max, state) =
  let (max', state1) = normalizeHC theo sfb clauses (max, state)
   in if max' == 0
        then (0, state1)
        else case tryHC theo sfb clause state1 of
          Nothing -> (max', state1)
          Just state2 -> normalizeHC theo sfb (clause : clauses) ((max' - 1), state2)

tryHC :: AlgTheo -> Int -> Hornclause -> State -> Maybe State
tryHC theo sfb (name, head, body) state =
  -- if head = ik we should create 2 rules
  case head of
    ...
    otherwise ->
      let vars = vars2Msg (concatMap var_fact body)
       in if vars == Var 77777777
            then
              if vars2Msg (concatMap var_fact [head]) == Var 77777777
                then listToMaybe $ successors False theo False sfb (\i -> (body, [head], [], (M 0 (Number 1) (Number 2) (Number 3) (BinOp opApply (Alby (name)) (Alby name)) (Number 5)) : head : body, False)) state
                else
...
successorsHC hcs very_lazy theo@(fecTheo, _) por sfb rule s =
  --- was: state
  let r = new_var_rename s (rule (getTime s))
   in map
        ((\x -> snd $ normalizeHC theo sfb hcs (100, x)) . fst)
        ( compList
            [ (analz_knowl very_lazy theo por) . (perform_step sfb),
              match_rest fecTheo,
              match_msg_fact very_lazy fecTheo,
