               ...
               in concatMap
                    ( \f ->
                        str
                          ++ (ppFact Pretty f)
                          ++ "\n"
                          ++ (traceback rulesh ik0 ik0 f ('-' : str))
                    )
                    newgoals
...
xscryptsub sigma =
  foldMsg
    ( \y ->
        if isJust (stripPrefix "XScrypt" y)
          then Atom "i"
          else sigma (Atom y)
    )
    (\f xs -> Comp f xs)
...
hornFP0 crefiners verify hr abstractlist opts header =
  let --- eliminate fresh variables
      ...
      rules' =
        if (outt opts) == FPI
          then
            if (length rules0') < (iterateFP opts)
   ...
   in if verify && (not (null attacks))
        then
          let refiners0 = map (drop 3) (filter (isPrefixOf "abs") (identsFL attacks))
              refiners = nub (refiners0 ++ crefiners)
           in if (length refiners) == (length crefiners)
  ...
  where
    ...
    abbi =
      if null (rulesH hr)
        then
          concatMap
            (newabsrule (makerefine refiners (absrulesH hr)) types)
            (absrulesH hr)
        else []
    ...
    dotypeabs x y = case (typeit types (Atom x), typeit types y) of
      (Comp Apply [Atom type1, _], Comp Apply [Atom type2, _]) ->
        if (isAtype (Atom type1))
          then
            if (isAtype (Atom type2))
              then
                if type1 == type2
                  then (checkEQtypes y)
                  else
                    error
                      ( "Ill-typed abstraction: "
                          ++ (show x)
                          ++ " vs. "
                          ++ (show y)
                          ++ ":"
    ...
    checkEQtypes (Comp Apply ((Atom "autoEquals") : x)) =
      case x of
        [Atom x] ->
          let xabbis = [z | (y, z) <- abbi, y == x]
           in if null xabbis
                then (Atom "autoEqualsRemoved")
...
ruleListFP refiners verify pts =
  ruleListFP0
    ...
    types
    abs
    (pts, map (toFPStateR False) (rules pts))
    ( "Protocol: "
        ++ protn
        ++ "\nTypes:\n"
        ++ (show ((Purpose, purposes) : types))
  ...
  where
    (protn, types, _, abbiGiven, _, goals) = (protocol pts)
    purposes =
      [ "purpose" ++ m | (ChGoal (_, ch, _) (Atom m)) <- goals, ch /= Insecure, ch /= Confidential
      ]
    hasAuthenticationGoals = not $ null purposes
    ags =
      if hasAuthenticationGoals && (authlevel (options pts) == Weak)
        then ["a", "b"]
        else ["a"]
    abbi =
      if null abbiGiven
...
data RefLev
  = Actor -- only the agent creating the value
...
statemsgs l r =
  let s =
        (concat [(map fst msgs) | (State _ msgs) <- (l ++ r)])
          ++ (concat [msgs | (FPState _ msgs) <- (l ++ r)])
   in if null s
        then error ("No state facts in " ++ (show (l ++ r)))
        else s
...
genuineAbsRule (l, [], [], r) =
  let trans = nub ((concatMap getGen l) ++ (concatMap getGen r))
   in if trans == []
        then [(l, [], [], r)]
        else
          if (length trans) > 1
            then error ("Abstraction failed (you may try to specify an abstraction manually or the classic module). Error details: multiple transformations in the genuine abstraction " ++ (show trans))
            else
              let [(Atom s, t)] = trans
                  sub = mksub [(s, t)]
...
ruleListFP0 ags crefiners verify types abstractlist (pts, rules) header =
  let --- eliminate fresh variables
      ...
      rules' =
        if (outt (options pts)) == FPI
          then
            if (length rules0') < (iterateFP (options pts))
              then error "Iteration finished"
              else irules
          else rules0'
      rules'' = map rule2fact2fact rules'
...
abstract_initial0 ags ((FPState r ((Atom name) : rest)) : other) =
  let other_names = nub [n | (Atom n) <- rest, isVariable n, n /= "SID"]
      ...
      insts [] sub = [sub]
      insts (n : names) sub =
        (insts names (addSub sub n (Atom "i")))
          ++ (concat [insts names (addSub sub n (Atom ag)) | ag <- ags])
   in [ subfact sub (FPState r ((Atom ag) : rest))
        | ag <- absname,
          sub <- insts other_names (addSub (\x -> x) name (Atom ag))
      ]
        ++ ( if isVariable name
               then
                 [ Iknows (sub m)
                   | sub <- insts other_names (addSub (\x -> x) name (Atom "i")),
                     m <- rest
                 ]
               else []
           )
...
intruderAna =
  [ ( [ Iknows (Comp Crypt [(Atom "K"), (Atom "M")]),
