...
substitute_constraint sub (ts, ik1, ik2) =
  ( map (substitute sub) ts,
    substituteMT sub ik1,
    substituteMT sub ik2
  )
...
gencheck0 fecTheo vars constraint@([], ik, nik) constraints@(cs, ineq) sub =
  ...
  if (isDFrom constraint) && checktypingvars
    then []
    else [((cs ++ [(vars, ik, nik)], ineq), sub)]
  where
    niktypes = typesMT nik
    checktypingvars =
      (all isTyped vars)
...
gencheck0 fecTheo@(_, theoryOf) vars constraint@(t : terms, ik, nik) constraints@(cs, ineq) sub =
  --- be lazy:
  if isSimple t
    then gencheck0 fecTheo (vars ++ [t]) (terms, ik, nik) constraints sub
    else
      ...
      if elemMT t nik
        then gencheck0 fecTheo vars (undiff (terms, ik, nik)) constraints sub
          ...
          if elemMT t ik
            then gencheck0 fecTheo vars (terms, ik, nik) constraints sub
            else
              let --- compute all unifiers with sth. in nik:
                  ...
                  comp_possibilities =
                    concatMap
                      ...
                      ( filter
                          (\(_, sub) -> (Map.toList sub) == [])
                          ( case t of
                              BinOp op m1 m2 ->
                                let otherops = theoryOf op
                                    ops = if otherops == [] then [op] else otherops
                                 in concatMap
                                      ( \op' ->
                                          if op' == (stringToOp "fst")
                                            then error "FSTbinop"
                                            else topdecF fecTheo op' t
                                      )
                                      (filter intrudible ops)
                              UnOp op m ->
...
substituteIKstate sub (ik, nik, b) =
  ( substituteIK sub ik,
    substituteIK sub nik,
    b
  )
...
mkinitialIKstate ik =
  if newImpl
    then (emptyIK, (listToMT ik, []), False)
...
analz0 theo@(fecTheo, (decanaTheo, _)) bool (constraints@(cs, ineq), sub, n : new, test, azed)
  ...
  | otherwise =
      if (isPair n)
        then
          let (BinOp _ m1 m2) = n
           in analz0 theo bool (constraints, sub, m1 : m2 : new, test, azed)
        else
          if (notOp n)
            then analz0 theo True (constraints, sub, new, test, addToMT azed n)
            else
              let gencheck_m m =
                  ...
                  tds1 =
                    map
                      ( \(k, m, sub') ->
                          (k, m, sub', substitute_constraints fecTheo sub' constraints)
                      )
                      tds0
                  ...
                  newnew sub =
                    ( if (tail tds') == []
                        then (:) (substitute sub m)
                        else
                          ((:) (substitute sub m))
                            . ((:) (substitute sub n))
                    )
                      (map (substitute sub) (new ++ test))
               in if tds' == []
                    then analz0 theo bool (constraints, sub, new, test, addToMT azed n)
                    else
                      if ps == []
                        then analz0 theo bool (constraints, sub, new, n : test, addToMT azed n)
                        else
                          if elem sub ps
                            then analz0 theo True (constraints, sub, newnew Map.empty, [], addToMT azed n)
                              ...
                              ( analz0
                                  theo
                                  True
                                  ( (fst constr', checkineqs fecTheo ((negsubs [] ps) ++ (snd constr'))),
                                    sub'',
                                    new,
                                    test,
                                    addToMT azed'' n
                                ...
                                ++ ( concatMap
                                       ( \sub' ->
                                           let constraints' = substitute_constraints fecTheo sub' constr'
                                               sub''' = mergesub sub' sub''
                                            in if isSatisfiable (snd constraints')
                                                 then
                                                   analz0
                                                     theo
                                                     True
