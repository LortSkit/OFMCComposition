...
splitOn0 key word buffer =
  if key `isPrefixOf` word
    then (reverse buffer) : (splitOn key (drop (length key) word))
...
processLine (line : rest) mapping =
  if line == ""
    then mapping
    else
      let newMapping = (addMapping (last (splitOn "->" (debughead "at processLine" (splitOn ":" line)))) (addMapping (debughead "at processLine/2" (splitOn "->" (debughead "at processLine/3" (splitOn ":" line)))) mapping))
...
printMessage [] _ _ _ = ""
printMessage (line : rest) mapping cnt swidth =
  if line == ""
    then ""
    else
      let fromIndex = fromJust (findInMapping (debughead "at printMsg" (splitOn "->" (debughead "at printMsg" (splitOn ":" line)))) mapping)
...
messageLength [] = 0
messageLength (line : rest) =
  let myLength = (length (last (splitOn ":" line)))
...
printAT str =
  let (mapping, sheight) = createMapping str
...
showattack result =
  let attacks = (debughead "at showattack" (splitOn "% Reached State:" (attack result)))
...
showComposableResult :: (ComposableResult, [String], [String], [String], [String], [String]) -> String
showComposableResult (res, pub, sec, faults, gsmpappterms, gsmpabstractchterms) =
  let explanation
        | res == Composable = ""
        ...
        -- \| res == AppIncompatible = ""
        | res == AbstractChIncompatible = "\nEXPLANATION: The GSMP(Ch#) has non-public intersection with the GSMP(App). \nNote: It's GSMPs, so for example 'sk(A,B)' is the same as 'sk(C,s)', since if one is in the GSMP, so is the other.\nWith that in mind, please compare the Pub set and the non-public GSMP(Ch#) and GSMP(App) overlap:"
...
parseArgs0 ("--untyped" : xs) (onp, anbonp) =
  if (typing onp) /= AsGiven
    then error ("Error parsing typing arguments.")
    else parseArgs0 xs (onp {typing = Untyped}, anbonp)
parseArgs0 ("-u" : xs) (onp, anbonp) =
  if (typing onp) /= AsGiven -- equal to untyped
    then error ("Error parsing typing arguments.")
    else parseArgs0 xs (onp {typing = Untyped}, anbonp)
parseArgs0 ("-strongly-typed" : xs) (onp, anbonp) =
  if (typing onp) /= AsGiven
    then error ("Error parsing typing arguments.")
...
parseArgs0 ("--of" : "FPI" : file : xs) (onp, anbonp) =
  --- for debugging: iterative
...
parseArgs0 (('-' : '-' : opt) : xs) (onp, anbonp) =
  if (isPrefixOf "output" opt)
    then
      parseArgs0 xs (onp, anbonp)
    else
      if (isPrefixOf "redirect" opt)
        then
          parseArgs0 xs (onp {output = Just (drop 9 opt)}, anbonp)
        else error ("unrecognized option '" ++ ('-' : '-' : opt) ++ "'")
parseArgs0 (f : xs) (onp, anbonp) =
  if (filename onp) == "" && (debughead "at ParseArg" f) /= '-'
...
getIntFromAssumeJust justDepth = case justDepth of
  Just dep -> dep
  Nothing -> error "Internal function 'getIntFromAssumeJust' expected a depth with structure Just [depth], got structure Nothing."
...
main =
  do
    ...
    ---
    putStr $ if (outt anbonp) /= IF && (outt anbonp /= Isa) && (outt anbonp /= AVANTSSAR) then productname ++ "\n" else "" --- or ASLan
    putStr $ if (vert anbonp) && (outt anbonp) /= IF then "Running with --vert flag with a depth of " ++ show (getIntFromAssumeJust (maxDepth anbonp)) ++ ".\n" else ""
    putStr $ if (outt anbonp) == IF then "% Generated by " ++ productname ++ (if (vert anbonp) then "% (With --vert flag enabled)\n" else "") else "" --- what the ... ?
    ...
    if ((outt anbonp) == Amphibian)
      then do
        ...
        ( case (decision classicresult, decision fpresult) of
            (ATTACK_FOUND, _) -> do
              (putStr (showresult classicresult (outt anbonp == AVANTSSAR)))
            ...
            _ -> do
              classicresult2 <- takeMVar classicResult
              ...
              ( case (decision classicresult2, decision fpresult2) of
                  ...
                  _ -> do
                    putStr (showresult classicresult2 (outt anbonp == AVANTSSAR))
...
iterativeFP (onp, anbonp) =
  do
    str <- readFile (filename onp)
    writeFile
      ((fromJust (anboutput anbonp)) ++ (show (iterateFP anbonp)))
...
mainWithArgs (onp, anbonp) (brothers :: Maybe ((MVar Result, MVar Result))) iterative =
  do
    ...
    (algTheo, tab) <-
      if tfile == ""
        then return ((fecStd, (decanaStd, cancellationStd)), Map.empty)
    ...
    if iterative && str == nothing_found
      then
        let cns = fromJust (numSess anbonp)
         in do
              putStr ("Verified for " ++ (show cns) ++ " sessions\n")
    ...
    ( let result =
       ...
       in case output onp of
            Nothing ->
              if isJust brothers
                then do
                  putMVar (fst (fromJust brothers)) result
                  putMVar (snd (fromJust brothers)) dummyresult
                  return ()
                else do
                  putStr (showresult result (outt anbonp == AVANTSSAR))
...
execore
  algTheo
  onp
  (init, rules, secr, goals, Just execG, hcs)
  isNewIF =
     ...
     in if must
          then error "MUST currently not supported"
            ...
            let check n = (wrids2 depthbound (general_attack_check algTheo cd [(\i -> execG n)]) (treeconstruct hcs algTheo must cd sfb (depthbound + n) (newstate, rules)))
                ---
                res n =
                  let rstr = fst $ check n
                   in if rstr == nothing_found
                        then False
                        else
...
core
  algTheo
  onp
  (init, rules, secr, goals, execCR, horntheo)
  isNewIF =
    let fname = filename onp
        sfb = sesrep onp
        pth = path onp
        ...
        newstate =
          if sessco onp
            then
              if horntheo /= []
                then error "hcs not yet supported by sessco"
                else
                  let (bool, mfacts) = xexec_check algTheo cd rules i (varstart, init_init algTheo init)
                   in if not bool
                        then
                          ...
                          let (mfacts', newfacts) = partition isMTerm (map uniquizef (mfacts))
                              ...
                              cs'' = if davars == [] then cs' else ((fst cs') ++ [constraint0], snd cs')
                           in ( init
                                  { facts = (facts init) ++ newfacts,
                                    ik = undiffIKstate ik',
                                    cs = cs''
                                  }
                              )
     ...
     in if must
          then error "MUST currently not supported"
            ...
            if isNewIF && bool && (sessco onp)
              then error "The protocol description has loops.\nPlease try without option --sessco\n"
              else
                ( if isNewIF && bool
                      ...
                      (Nothing, Nothing) ->
                        let (str, stats) =
                              ( neusuch
                                  ...
                                  rules
                                  horntheo
                                  (general_attack_check algTheo cd goals)
                                  nodemax
                                  depthbound
                                  sfb
                                  ([(varstart, newstate)], [])
                         ...
                         in if (str == reached_balance)
                              then
                                let maxdepth = if isJust depthb then fromJust depthb else depthbound
                                    (a, b) = core algTheo (onp {depth = Just maxdepth}) (init, rules, secr, goals, execCR, horntheo) isNewIF
                                 in (a, b)
                              else (str, stats)
                      (Just list, Nothing) ->
                        ( shn2
                            (general_attack_check algTheo cd goals)
                            (\depth -> treeconstruct horntheo algTheo must cd sfb depth (newstate, rules))
                            list,
                          init_stats
                        )
                      (Just [], Just ply) ->
...
showrule r =
  let (lhs, neg, cond, rhs, rtype) = r 0
   in ( (show lhs)
          ++ "\n"
          ++ (show neg)
          ++ "\n"
...
decanaStd =
  [ ---Extend
    ( BinOp opCrypt (UnOp opInv (Var x1)) (Var x2),
      [Tauto],
      (Var x1),
      (Var x2)
    ),
    ---Extend
    ( BinOp opCrypt (Var x1) (Var x2),
      [Negsub [(-1)] [(Var x1, UnOp opInv (Var (-1)))]],
      UnOp opInv (Var x1),
      (Var x2)
    ),
    --- Replace
...
cancellationStd =
  ...
  where
    f x
      ...
      | x == opCrypt =
          Just
            [ (BinOp opCrypt (UnOp opInv (Var x1)) (BinOp opCrypt (Var x1) (Var x2)), (Var x2)),
      ...
      | x == opXor =
          Just
            [ (BinOp opXor (Var x1) (Var x1), Atom 0 "e"),
...
irreduc op t =
  case theoryOf op of
    Just theo ->
      map
        ( \(lhs, _) ->
            let lhs' = rename (-30) lhs
             in Negsub (msg_vars lhs') [(t, lhs')]
        )
        theo
    Nothing -> []
