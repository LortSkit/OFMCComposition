...
stringToOp0 string =
  let loop [] (n :: Integer) = n
   ...
   in if (length string) > 10 && (not grace)
        then
...
substitutev sub vars (Var v)
  | v `elem` vars = (Var v)
  | otherwise =
      let m = fromMaybe (Var v) (Map.lookup v sub)
       in if disjoint (msg_vars m) vars
            then m
